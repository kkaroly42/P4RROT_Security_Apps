...
fp = FlowProcessor(
        ...
        standard_fields = [SrcIp, TcpDstPort]
    )
(
fp
.add(CheckControlPlaneSet(["hdr.ipv4.src"], "unregistered"))
.add(LogicalNot("unregistered"))
.add(If("unregistered"))
    .add(CheckControlPlaneSet(["hdr.tcp.dstPort"], "allowed"))
    .add(If("allowed"))
        .add(Digest(["hdr.ipv4.src"], ["hdr.tcp.dstPort"]))
    .EndIf()
    .add(DropPacket())
.EndIf()
)
